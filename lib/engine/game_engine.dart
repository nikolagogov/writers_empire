...
class GameEngine {
  ...
  // 🔧 FIX: Renamed from unlockedBooks to lockedBooks (logic is correct)
  List<Book> get lockedBooks => BOOKS.where((b) => !booksOwned.contains(b.id)).toList();
  ...
  void checkAchievements() {
    // First Word
    if (totalWords >= 1 && !achDone.containsKey('first_word')) {
      achDone['first_word'] = true;
    }
    // 1K words
    if (totalWords >= 1000 && !achDone.containsKey('first_k')) {
      achDone['first_k'] = true;
    }
    // 1M words
    if (totalWords >= 1000000 && !achDone.containsKey('first_m')) {
      achDone['first_m'] = true;
    }
    // First Retire
    if (muses >= 1 && !achDone.containsKey('first_retire')) {
      achDone['first_retire'] = true;
    }
  }
